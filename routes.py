...
def dashboard():
    ...
    for hc in healthchecks:
        ...
        latest_result = CheckResult.query.filter_by(
...
def get_healthcheck_results(hc_id):
    ...
    results = CheckResult.query.filter_by(
        healthcheck_id=hc_id
    ).order_by(CheckResult.id.desc()).paginate(
...
def create_alert_config(hc_id):
    ...
    try:
        ...
        logger.info(f"Created alert config for {healthcheck.name}: {data['alert_type']}")
