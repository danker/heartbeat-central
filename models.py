...
class Healthcheck(db.Model):
    ...
    # Relationships
    check_results = db.relationship('CheckResult', backref='healthcheck', lazy=True, cascade='all, delete-orphan')
    alert_configs = db.relationship('AlertConfig', backref='healthcheck', lazy=True, cascade='all, delete-orphan')
...
class CheckResult(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    healthcheck_id = db.Column(db.Integer, db.ForeignKey('healthcheck.id'), nullable=False)
