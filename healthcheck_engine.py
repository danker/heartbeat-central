...
class HealthcheckEngine:
            ...
            if response.status_code >= 200 and response.status_code < 300:
                ...
                if healthcheck.expected_text:
                    if healthcheck.expected_text.lower() in response.text.lower():
                        ...
                        logger.info(f"✓ {healthcheck.name}: Healthy (found expected text)")
                    else:
                        ...
                        logger.warning(f"✗ {healthcheck.name}: Unhealthy (expected text not found)")
                else:
                    result.status = HealthcheckStatus.HEALTHY
                    logger.info(f"✓ {healthcheck.name}: Healthy (HTTP {response.status_code})")
            else:
                result.status = HealthcheckStatus.UNHEALTHY
                result.error_message = f"HTTP {response.status_code}: {response.reason}"
                logger.warning(f"✗ {healthcheck.name}: Unhealthy (HTTP {response.status_code})")
                
        except requests.exceptions.Timeout:
            result.status = HealthcheckStatus.UNHEALTHY
            result.error_message = f"Request timeout after {healthcheck.timeout} seconds"
    ...
    def _check_and_send_alerts(self, healthcheck, current_result):
        ...
        if current_result.status == HealthcheckStatus.UNHEALTHY:
            ...
            last_result = CheckResult.query.filter_by(
                healthcheck_id=healthcheck.id
            ).order_by(CheckResult.id.desc()).offset(1).first()
